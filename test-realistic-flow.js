...
console.log('Testing realistic lobby to game flow...');
...
ws1.on('message', function incoming(data) {
  ...
  if (message.type === 'GAME_CREATED') {
    ...
    // Instead of closing the connection, simulate navigating to game page
    // while keeping the lobby connection alive for a moment
    console.log('Player 1 navigating to game page...');
    
    // Create a new connection for the game (simulating the multiplayer page)
    setTimeout(() => {
      const gameWsUrl = `ws://localhost:8080?id=${playerId1}&name=${encodeURIComponent(playerName1)}&lobby=true`;
      console.log('Player 1 connecting to game:', gameWsUrl);
